...
try:
    ...
    if os.path.exists(DB_PATH):
        ...
        print("⚠️ Database file not found (will be created on first archive)")
except Exception as e:
...
print("\n[TEST 3] Price Data Collection")
print("-" * 60)
print("⚠️ This will fetch real data from TCBS API")
...
print("\n[TEST 4] Alert System")
print("-" * 60)
print("⚠️ This will check alerts and may send Telegram messages")
response = input("Continue? (y/n): ")
if response.lower() == 'y':
    try:
...
print("\n[TEST 5] Data Archival")
print("-" * 60)
print("⚠️ This will move old data from Sheets to SQLite")
response = input("Continue? (y/n): ")
if response.lower() == 'y':
    try:
...
print("\n[TEST 6] Data Aggregation")
print("-" * 60)
print("⚠️ This requires historical data in SQLite")
response = input("Continue? (y/n): ")
if response.lower() == 'y':
    try:
